  ...
  useEffect(() => {
    ...
    const checkBudgets = () => {
      ...
      if (jsSize > BUDGETS.js) {
        console.warn(`⚠️ JS bundle size exceeded budget: ${(jsSize / 1024).toFixed(2)} KB > ${(BUDGETS.js / 1024).toFixed(2)} KB`);
      }
      
      if (cssSize > BUDGETS.css) {
        console.warn(`⚠️ CSS bundle size exceeded budget: ${(cssSize / 1024).toFixed(2)} KB > ${(BUDGETS.css / 1024).toFixed(2)} KB`);
      }
      
      const totalSize = jsSize + cssSize;
      if (totalSize > BUDGETS.total) {
        console.warn(`⚠️ Total bundle size exceeded budget: ${(totalSize / 1024).toFixed(2)} KB > ${(BUDGETS.total / 1024).toFixed(2)} KB`);
      }
