	...
	return (
		<div style={{ border: "1px solid #e2e8f0", borderRadius: 8, overflow: "hidden" }}>
			<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.5rem 0.75rem", background: "#f8fafc", borderBottom: "1px solid #e2e8f0" }}>
				<button onClick={() => setCurrent(new Date(current.getFullYear(), current.getMonth() - 1, 1))} style={navBtn} aria-label="Previous month">‹</button>
				<div style={{ fontWeight: 600 }}>{monthLabel}</div>
				<button onClick={() => setCurrent(new Date(current.getFullYear(), current.getMonth() + 1, 1))} style={navBtn} aria-label="Next month">›</button>
			</div>
			<div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2, padding: 8 }}>
				{"SMTWTFS".split("").map((l) => (
					<div key={l} style={{ textAlign: "center", color: "#64748b", fontSize: 12 }}>{l}</div>
				))}
				{days.map((d) => {
					const isOtherMonth = d.getMonth() !== current.getMonth();
					const selected = (range.start && isSameDay(d, range.start)) || (range.end && isSameDay(d, range.end));
				...
				<div style={popupBackdrop} onClick={() => setPickerDay(null)}>
					<div style={popup} onClick={(e) => e.stopPropagation()}>
						...
						<div style={{ display: "grid", gap: 8 }}>
							<label style={label}>Start date<input type="date" defaultValue={toInputDate(pickerDay)} style={input} id="start-date" /></label>
							<label style={label}>End date<input type="date" defaultValue={toInputDate(pickerDay)} style={input} id="end-date" /></label>
						</div>
						<div style={{ display: "flex", gap: 8, marginTop: 12, justifyContent: "flex-end" }}>
							<button onClick={clearSelection} style={secondary}>Clear</button>
							<button
								onClick={() => {
